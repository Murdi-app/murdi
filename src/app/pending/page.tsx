...
export default function PendingPage() {
  ...
  // لا يوجد اشتراك في المنصة — مبلغ ثابت لكل خدمة وأتعاب نجاح. وكان النص
  // هنا يقول «رسوم الاشتراك» و«جدّد اشتراكك»، فيَعِد العميل بنموذج تسعير
  ...
  const content: Record<Status, { icon: string; title: string; msg: string }> = {
    ...
    expired: { icon: '', title: 'الحساب غير مفعّل', msg: 'انتهت صلاحية تفعيل حسابك. تواصل معنا لإعادة تفعيله.' },
  }
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .pd-icon { width:80px; height:80px; background:linear-gradient(135deg,#1A3D34,#1A3D34); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:36px; margin:0 auto 24px; box-shadow:0 10px 28px rgba(46,158,123,0.28); }
        .pd-logo { font-family:'Amiri',serif; font-size:26px; color:#1A3D34; margin-bottom:6px; }
        .pd-company { color:#1A3D34; font-size:13px; background:#E8F5EF; display:inline-block; padding:5px 18px; border-radius:30px; font-weight:600; margin-bottom:28px; }
        ...
        .pd-btn { background:linear-gradient(135deg,#1A3D34,#1A3D34); color:#fff; border:none; padding:14px 40px; border-radius:40px; font-family:'Cairo',sans-serif; font-size:15px; font-weight:700; cursor:pointer; box-shadow:0 8px 22px rgba(46,158,123,0.28); }
