...
async function generateIPOAnalysis(data: Record<string, unknown>, score: number, market: string, stYears: number, reqYears: number): Promise<{ obstacles: string[]; plan: string[]; months: number } | null> {
  ...
  const prompt = 'انت محلل جاهزية طرح عام (IPO) وفق منهجية د. عبدالحكيم المرضي — مستشار سعودي معتمد خبير بمتطلبات هيئة السوق المالية وتداول. '
    ...
    + 'months: رقم واحد = نهاية آخر مرحلة في خطة الطريق (plan) التي ستكتبها، اي الشهر الذي تكتمل عنده آخر مرحلة. يجب ان يطابق months نهاية الاطار الزمني لآخر مرحلة بالضبط حتى لا يتعارض الرقم المعروض مع الخطة. التزم بهذه الحقائق النظامية الصارمة كحد ادنى: استكمال كل سنة قوائم مدققة ناقصة = 12 شهرا لكل سنة (الرئيسي 3 سنوات، نمو سنتين)؛ تعيين مراجع خارجي من الصفر = 6 اشهر؛ بناء حوكمة ومجلس ولجنة مراجعة = 9 اشهر؛ تحقيق ربحية مستدامة لسنة كاملة ان كانت خاسرة = 24 شهرا؛ التعثر في السداد = 36 شهرا على الاقل. اجمع المدد المتداخلة بذكاء (المتوازي لا يجمع، والمتتابع يجمع) واعطِ رقما واحدا واقعيا ومحافظا. المبالغة في التفاؤل تضر المصداقية. '
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (deep !== null) {
      ...
      // المدة = نهاية آخر مرحلة في خطة Claude (متسقة مع الخطة المعروضة، لا تتذبذب)
      if (deep.months > 0) monthsToReady = deep.months;
    }
  } catch {}
