...
export async function POST(req: Request) {
  ...
  const prompt = 'أنت تكتب نيابةً عن د. عبدالحكيم المرضي — مستشار مالي سعودي معتمد، دكتوراه إدارة أعمال، عضوية البورد الأمريكي، 15 سنة خبرة، عبر شركة حلول المرضي للاستشارات المالية. '
    ...
    + (si && si.inputs && Object.keys(si.inputs).length > 0 ? (
        ...
        + ((si.inputs as any).advisor_notes || ((si.inputs as any).years && (si.inputs as any).years['1'] && (si.inputs as any).years['1'].advisor_notes) ? ('\\n\\nملاحظات المستشار (إجابات العميل على الأسئلة): ' + ((si.inputs as any).advisor_notes || (si.inputs as any).years['1'].advisor_notes) + '\\nاستخدم هذه الإجابات لإعادة تصنيف أي فرق غير مفسَّر في مكانه المحاسبي الصحيح تلقائياً (إيداع مالك → حساب مالك جاري، أرباح سابقة → أرباح مرحّلة، قرض → التزام، أصول → أصول)، ولا تترك الفرق معلّقاً إذا فسّرته الملاحظات.') : '')
        + '\\n'
        ...
        + 'اطرح مخصص الديون المشكوك فيها (doubtful_debt) من الذمم المدينة لإظهار صافيها في المركز المالي، واحمل المخصص كمصروف في قائمة الدخل. عامل مصروف الإهلاك (depreciation) دائماً كبند مستقل غير مضمّن في المصروفات التشغيلية: اطرحه مرة واحدة فقط في قائمة الدخل، وأضِفه مرة واحدة في التدفقات التشغيلية كبند غير نقدي. لا تعرض نسختين للأرباح ولا تتردد في معالجته. وأوصِ صراحةً في الإيضاحات بإعداد جدول أعمار الذمم المدينة عند الاعتماد (كمستند مساند لا يدخل في متن القوائم). قدّم القوائم مكتملة بالأرقام لا كهيكل فارغ. أي بند غير مُدخل، عالجه بمنطق محاسبي سليم أو أشر إليه صراحة كبند يحتاج استكمالاً، دون اختلاق رقم. وإن كانت بيانات العميل ناقصة (عنده الإيراد فقط مثلاً) فابنِ ما يمكن وأضِف قسماً بعنوان (أسئلة ترفع دقة قوائمك وربحك الحقيقي) يوجّهه لكشف بنود مشروعة تحسّن صورته بصدق: مصاريف شخصية للمالك محملة على المنشأة يجب فصلها، مبيعات نقدية غير موثّقة يجب تسجيلها، أصول مملوكة غير مُدرجة، ومخزون غير مجرود. الهدف إظهار الربح الفعلي كاملاً لا تضخيمه، وكل رقم يجب أن يكون موثّقاً للبنك.\\n'
      ) : '')
