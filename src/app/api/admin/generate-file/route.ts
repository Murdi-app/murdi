...
export async function POST(req: Request) {
  ...
  try { const b = await req.json(); companyId = String(b.company_id || ''); track = (b.track === 'investment' || b.track === 'acquisition' || b.track === 'valuation' || b.track === 'negotiation') ? String(b.track) : 'funding'; region = String(b.region || ''); const fa = Number(b.funding_amount); if (fa > 0) fundingAmount = fa; }
  ...
  try {
    try {
      ...
      if (track !== 'acquisition' && track !== 'valuation' && track !== 'negotiation' && (at === 'investment' || at === 'funding')) track = at;
    } catch {}
    const content = await generateFileContent(client, track as 'funding' | 'investment' | 'acquisition' | 'valuation' | 'negotiation', region);
    const html = buildFileHTML(client, content, track as 'funding' | 'investment' | 'acquisition' | 'valuation' | 'negotiation', region, statementsHtml);
