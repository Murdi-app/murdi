...
export async function GET() {
  ...
  try {
    ...
    // ★ والمستبعد والموقوف لا يُعرضان لموظفة — وهذا ما كان ناقصاً (١٤ سبتمبر):
    //   كنتُ أضع الصفّ «مستبعدة» أو «موقوفة» وأحسبه اختفى، واللوحة لا تقرأ
    //   الحالة أصلاً فيبقى معروضاً. فظهرت لضي جهةُ تمويل شاحنات على ملفٍ
    //   مسارُه نقد، وظهرت أبوابُ المالك الشخصية (أجيل · منافع · رقمية ·
    //   التيسير · فندنق سوق) وهي التي نُهي عن مخاطبتها من المكتب.
    ...
    const clients = (cos || []).map((c: { id: string; company_name: string | null; city: string | null; sector: string | null }) => {
      const rows = (byCo.get(c.id) || []).map((m: Row) => {
        ...
        // ★ وبابُ الهاتف لا يظهر إلا بتوجيهٍ مكتوب من المكتب. فالصفّ بلا
        //   توجيه مسوّدةٌ قديمة لا مهمّة: ظهرت لضي جهةُ تمويل شاحنات على
        //   ملفٍ مسارُه نقدٌ فقط بأمر المالك — صفٌّ من أغسطس بقي في الجدول،
        //   وفتحتُه عليها حين جعلتُ أبواب الهاتف تظهر بلا إرسال. والمكالمة
        //   التي لا يعرف صاحبها ماذا يقول فيها ليست عملاً، فهذا الشرط يمنع
        //   الحالتين بقاعدةٍ واحدة.
        const phoneDoor = String(m.contact_method || '') === 'هاتف'
          && String(m.office_hint || '').trim() !== '';
