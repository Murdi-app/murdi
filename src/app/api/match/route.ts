...
export async function POST() {
  ...
  type WebOffer = { region?: string; provider: string; product: string; requirements: string; source: string; verdict?: string; gaps?: string[]; amountRange?: string; timeline?: string; saudiPrecedent?: string | null; legalPath?: string | null };
  ...
  try {
    ...
    const prompt = 'أنت محلل تمويل خبير في السوق السعودي والخليجي. ابحث في الويب بحثاً دقيقاً ومهنياً عن المنتجات التمويلية المتاحة حالياً للشركة، وقسّم بحثك إلى ثلاث طبقات بالأولوية:\nالطبقة الأولى (الأهم والأوسع): الجهات السعودية المرخّصة من البنك المركزي السعودي (ساما) — ابدأ بها وأعطها أكبر نصيب.\nالطبقة الثانية: جهات خليجية قد تموّل شركة سعودية (لها فروع في السعودية أو تموّل عبر الحدود).\nالطبقة الثالثة: جهات تمويل دولية قد تموّل شركات في السعودية أو الأسواق الناشئة.\nالجهات المرجعية لكل طبقة:\n' + LICENSED + '\n\n'
      ...
      + '4) طابق المنتجات مع تشخيص النشاط أعلاه بدقة: اقترح تمويل نقاط البيع فقط إن كان "نقاط بيع = نعم"؛ تمويل الفواتير/المستخلصات فقط إن كان "يصدر فواتير = نعم"؛ تمويل الأسطول/المعدات فقط إن كان "لديه أسطول = نعم". إن كانت الإجابة لا، لا تقترح ذلك النوع إطلاقاً مهما بدا مناسباً.\n\n'
      ...
      + 'ابحث براحتك وأرجع كل العروض المناسبة فعلاً عبر الطبقات الثلاث دون التقيّد بعدد معيّن — الأولوية للسعودية (الأكثر)، ثم الخليج، ثم الدولي. رتب داخل كل طبقة من الأنسب للأقل، ولا تُدرج عرضاً غير مناسب لمجرد زيادة العدد.';
