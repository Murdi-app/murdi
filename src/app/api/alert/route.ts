...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      html: `<div dir="rtl" style="font-family:Tahoma,Arial;padding:0;background:#13302A;color:white;max-width:600px;margin:0 auto;border-radius:16px;overflow:hidden;">
        <div style="background:linear-gradient(135deg,#13302A,#13302A);padding:28px;text-align:center;border-bottom:2px solid #C9A84C;">
          <div style="color:#C9A84C;font-size:24px;font-weight:bold;letter-spacing:2px;">MURDI</div>
          <div style="color:#8aa;font-size:13px;margin-top:4px;">مستشارك المالي الذكي</div>
        </div>
        <div style="padding:28px;">
          <h2 style="color:#C9A84C;font-size:18px;margin:0 0 16px;">🔔 تنبيه ذكي عن ${companyName}</h2>
          <div style="background:#13302A;padding:20px;border-radius:12px;border-right:4px solid #C9A84C;color:#fff;font-size:15px;line-height:1.9;">${alertText}</div>
          <div style="display:flex;gap:12px;margin:24px 0;flex-wrap:wrap;">
            <div style="flex:1;min-width:120px;background:#13302a;padding:14px;border-radius:10px;text-align:center;">
              <div style="color:#8aa;font-size:11px;">أيام البقاء</div>
              <div style="color:#C9A84C;font-size:22px;font-weight:bold;">${survivalDays || '-'}</div>
            </div>
            <div style="flex:1;min-width:120px;background:#13302a;padding:14px;border-radius:10px;text-align:center;">
              <div style="color:#8aa;font-size:11px;">Murdi Score</div>
              <div style="color:#C9A84C;font-size:22px;font-weight:bold;">${murdiScore || '-'}/85</div>
            </div>
          </div>
          <a href="https://murdi.sa/goal" style="display:block;background:linear-gradient(135deg,#C9A84C,#C9A84C);color:#13302A;text-align:center;padding:14px;border-radius:10px;text-decoration:none;font-weight:bold;font-size:15px;">ادخل لمراجعة التحليل الكامل ←</a>
        </div>
        <div style="padding:16px;text-align:center;color:#5B7068;font-size:11px;border-top:1px solid #13302a;">Murdi — بُني على منهجية د. عبدالحكيم المرضي</div>
      </div>`
    })
