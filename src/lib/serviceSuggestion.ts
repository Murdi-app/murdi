...
export const SERVICES: Record<string, ServiceDef> = {
  'تمويل العقد': {
    definition: 'قراءة عقدٍ بعينه — مُرسى أو قبل الترسية — لاستخراج فجوته النقدية بالمستخلصات شهراً بشهر، وحصر ضماناته (عطاء ودفعة مقدمة وحسن أداء وبدل محتجزات) بمبالغها ومواعيدها وغطائها النقدي، وتركيب هيكل تمويله من أكثر من جهة، وقراءة بنوده التي تصنع الفجوة بأثر كل بند بالريال، ثم مخاطبة الجهات والتفاوض حتى يصدر الضمان أو يُصرف التمويل. والموضوع هنا عقدٌ لا منشأة.',
    output: 'خريطة نقدية شهرية حتى الإفراج عن المحتجز مع أعمق نقطة ورأس المال العامل المطلوب، وجدول «نفس العقد بشروط أخرى»، وحزمة الضمانات، وهيكل تمويل مركَّب لكل لحظة من لحظات العقد، وجدول الجهات المرشّحة بفجواتها، وقراءة البنود مع ما يُطلب تعديله.',
    tracks: ['funding'],
  },
  ...
  // ── العنوان المعياري كما في الفهرس ───────────────────────────────
  //
  // دُمج الاسمان القديمان («إعادة الهيكلة المالية…» و«إعادة جدولة الديون»)
  // في عنوانٍ واحد بالفهرس، وبقي هذا السجلّ على الاسمين. فصار طلب العميل
  // يُخزَّن بالعنوان الجديد، ويُبحث عنه هنا فلا يوجد — فيُولَّد مخرَجه بلا
  // تعريف ولا وصفٍ لما يجب أن يحتويه. والاسمان القديمان باقيان أدناه
  // لأن لهما صفوفاً قائمة واقتراحات تشير إليهما.
  'إعادة هيكلة الالتزامات ومعالجة التعثّر': {
    definition: 'إعادة جدولة الديون، وقف النزيف النقدي، واستعادة انتظام السداد — سواءٌ وقع التعثّر أو كان الضغط النقدي يسبقه.',
    output: 'خطة تعافٍ مبنية على أرقام الشركة: تحليل الالتزامات القائمة، سيناريو الجدولة المقترح وأثره على التدفق النقدي، ضبط المصروفات، وترتيب أولويات السداد.',
    tracks: ['funding'],
  },
  ...
  'تجهيز ملف التمويل والتفاوض': {
    definition: 'إعداد الملف التمويلي الذي يُقدَّم للبنوك وجهات التمويل، وعرضه عليها بتكليفٍ منك، والمتابعة حتى يصل ردّها.',
...
export function suggestService(fd: SuggestInput, track: Track, score: number): ServiceSuggestion {
  ...
  if (track === 'funding') {
    return { urgency: 'recommended', icon: '🏦', service: 'تجهيز ملف التمويل ومخاطبة الجهات', why: 'الشركة مؤهّلة. تجهيز الملف احترافياً وعرضه على الجهات التي تنطبق عليها شروطها يرفع فرص القبول — رسمٌ ثابت مقابل عملٍ يُسلَّم، لا نسبةَ من التمويل.' };
  }
  if (track === 'investment') {
    return { urgency: 'recommended', icon: '📈', service: 'تجهيز ملف عرض المستثمر', why: 'الشركة جاذبة. تجهيز ملف العرض وترتيب أرقامه يرفع فرص الصفقة وبتقييم أفضل — رسمٌ ثابت مقابل عملٍ يُسلَّم.' };
  }
