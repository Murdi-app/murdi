...
export interface FileClientData {
  ...
  // ★ ثلاث حقائق ائتمانية كانت خارج المولّد كلياً، وهي في ملفٍ كملف منارة
  //   الشبكات أقوى ما فيه. فخرجت الوثيقة تبيع «صافي ربح ٢٠٨٬٩٢٠» على إيراد
  //   ٩٫٢ مليون — هامش ٢٫٣٪ تقرؤه اللجنة ضعفاً — وتسكت عمّا يحمل الطلب:
  //
  //   • خلوّ الذمة من أي التزام بنكي — أول ما يُسأل عنه في لجنة الائتمان
  //   • شركة أمّ قادرة على الضمان — تحوّل منشأةً صغيرة إلى ذراع مجموعة، وهي
  //     الباب السادس في آلية الممر الأجنبي وأقوى أبوابها
  //   • الملكية — تكشفها الجهة من السجل حتماً، وعرضُها مؤطَّرةً خيرٌ من أن
  //     تُكتشف مفاجأةً بعد أن يُفتح الملف
  /** لا التزامات بنكية قائمة — تُمرَّر true فقط حين تكون صحيحة */
  debtFree?: boolean;
  /** بلد الشركة الأم، وهل تقدر على الضمان */
  parentCountry?: string;
  parentCanGuarantee?: boolean;
  /** الملكية كما سُجّلت، وجنسية المالك إن وُجدت */
  ownership?: string;
...
export async function generateFileContent(
  ...
  const lines = [
    ...
    // ★ صافي الربح لا يُذكر في ملفٍ يخرج إلى جهة تمويل (قراره 2026-09-08).
    //
    // فالممول يقرأ القوائم المدققة بنفسه ويكوّن رأيه، ولا حاجة أن تلفت وثيقتنا
    // نظره إلى أضعف رقم فيها. وقع في ملف منارة الشبكات: صافي ربح 208,920 على
    // إيراد 9.2 مليون — هامش 2.3٪ — عُرض «تأكيداً على الربحية»، فصار الوثيقة
    // تدعو اللجنة إلى السؤال الذي يُسقط الطلب: كيف يخدم هذا الربح تسهيلاً بمليون؟
    //
    // وليس إخفاءً: القوائم تصل الجهة كاملةً، والرقم أمامها. وإنما لا يُبرَز.
    // ولا يُنفى أبداً — والقاعدة (9) في FUND_RULES تمنع الإفصاح السلبي أصلاً.
    //
    // ويبقى في مسار الاستثمار: المستثمر يشتري حصةً في ربح، فالربح هناك هو
    // الموضوع لا التفصيل — وقاعدة INVEST_RULES (6) تأمر بتصديره.
    (client.profit && isInvestment) ? 'صافي الربح: ' + num(client.profit) : '',
    ...
    client.debtFree ? 'لا توجد عليه أي التزامات تمويلية بنكية قائمة — سجلّ ائتماني نظيف (حقيقة مؤكَّدة من بياناته، أبرزها)' : '',

    // ★ الشركة الأم لا تُذكر في الوثيقة (قراره 2026-09-08) — تُدَّخر ورقةً
    //   تُلعب حين تسأل الجهة عن ضمانٍ إضافي، لا تُبذل في أول صفحة.
    //   ومن يفتح بتعزيزه الائتماني يكون قد تنازل قبل أن يُسأل.
    //   والحقلان يبقيان في `FileClientData` ليُستعملا في المخاطبة حين تُطلب.

    // ★ والملكية تُصاغ بلا جنسية: «شركة سعودية بملكية مستثمر» — وهي صحيحة
  ...
  const FUND_RULES = 'قواعد إلزامية لملف التمويل: (1) ممنوع منعاً باتاً ذكر درجة الجاهزية أو أي رقم أو تقييم داخلي لمنصة مُرضي — رقم داخلي لا يُعرض على جهة تمويل إطلاقاً. (2) ممنوع منعاً باتاً أي صيغة وساطة: لا «نيابةً عن العميل» ولا «on behalf of» ولا ما يوحي بأننا نتفاوض أو نتعاقد بدلاً عنه. الصيغة الصحيحة أننا جهّزنا ملف العميل ورفعنا جاهزيته وندعو الجهة لدراسته، ويبقى قرار التقديم والتعاقد بيد العميل وحده. (3) ممنوع أن يظهر المستشار كمن يحدد طلب العميل: المبلغ طلب العميل مبنيٌّ على خطته، لا توصية منا. (4) الالتزامات القائمة تُذكر كاملة وبصدق — أي تمويل قائم وأي إيجار تمويلي أو تأجير منتهٍ بالتملّك وأي قسط شهري وارد في البيانات — لأن السجل الائتماني سيكشفها، ونقصها يهدم المصداقية. (5) في theRequest فصّل أوجه استخدام المبلغ كما وردت حرفياً في غرض التمويل: كم للأصول وكم لرأس المال العامل وكم للتشغيل، ولا تكتفِ بوصف عام مثل «تعزيز الطاقة التشغيلية». (6) ابنِ نقاط القوة على وقائع العميل المذكورة في البيانات — أسماء عملائه وعقوده الموقّعة وأصوله المملوكة ودورة تحصيله — لا على كلام عام عن نمو القطاع أو الرؤية أو موقع المدينة. أي جملة تصلح لأي منشأة أخرى فهي حشو يُحذف. (7) لا تذكر رقماً لم يرد في البيانات ولا تقدّر رقماً بنفسك. (8) في الخاتمة اذكر وسيلة تواصل صريحة: البريد partners@murdi.sa. (9) ممنوع منعاً باتاً الإفصاح السلبي: لا تذكر ما لا يوجد في البيانات ولا تعلّق على نقص المعطيات — لا تكتب «لا تتضمن البيانات المتاحة كذا» ولا «لم تُدرج أرقام كذا» ولا أي جملة عن منهجية التحرير أو حدوده. اكتب ما لدى العميل فقط، واسكت عمّا سواه؛ فالإشارة إلى غياب معلومة تلفت نظر الممول إلى فجوة وتوحي بإخفاء. (10) وينطبق البند (4) على ما ورد في البيانات فعلاً: إن وردت التزامات فاذكرها كاملة، وإن لم ترد فلا تنفِ وجودها. (11) اسم الشركة بالعربية يُكتب دائماً «حلول المرضي للاستشارات المالية» بلا تغيير. (12) ممنوع منعاً باتاً ذكر الربح أو الربحية بأي صورة: لا رقم صافي ربح، ولا هامش ربح، ولا نسبة مئوية مشتقة من الربح، ولا وصف كيفي للربحية مثل «ربحية مستمرة» أو «هامش صحي» أو «قدرة على تحقيق نتائج إيجابية» أو «انضباط في التكاليف» أو «كفاءة تشغيلية تنعكس على الهامش». الجهة تقرأ القوائم المدققة بنفسها وتكوّن رأيها، ولا تلفت وثيقتنا نظرها إلى هذا الباب. وفي المقابل — ولهذا صلة بالبند (9) — ممنوع أيضاً نفي الربحية أو التعليق على غيابها أو الإشارة إلى أن الأرقام غير مذكورة: اكتب عن الإيراد والعقود والعملاء والأصول ودورة التحصيل، واسكت عن الربح سكوتاً تاماً. (13) قدرة السداد تُبنى على مصدر السداد لا على الربح: التدفقات المتولدة من العقود والفواتير والمستخلصات المصدَّرة على العملاء. ENGLISH-VERSION HARD RULES (apply verbatim when writing in English): (a) The firm name is written EXACTLY as "Holol Almurdi Financial Consulting" — never Holool, never Holul, never Al-Murdi, never any other transliteration, and never invent a variant. (b) The phrase "on behalf of" is FORBIDDEN anywhere in the document, and so are "acting for", "representing the client" and "negotiating for". Write instead that Holol Almurdi prepared and structured this financing file and invites the institution to review it, and that the decision to apply and to contract rests solely with the establishment. (c) Write every client, customer and counterparty name exactly as it appears in the client data — do not re-transliterate, correct or anglicise any proper name. ';
