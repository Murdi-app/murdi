...
export async function runDailyHunt(): Promise<{ total: number; byCategory: Record<string, number> }> {
  ...
  const axes = [
    ...
    { label: 'الطرح — رغبة مبكّرة لدى مسؤولي شركات', category: 'ipo_early_intent', count: 10, instruction: 'زاوية ذكية: لا تبحث عن شركة جاهزة للطرح (هذه عرفت طريقها وفات أوانها). ابحث عن مسؤولي/ملّاك شركات سعودية متوسطة (فوق مليونين) سألوا أو لمّحوا — مباشرة أو غير مباشرة — عن الطرح أو التحول لمساهمة أو الإدراج في نمو، في تعليقات أو مقابلات أو منشورات حديثة، بشكل يكشف رغبة مبكّرة واستعداداً ذهنياً لكنهم لم يبدأوا التجهيز بعد. احسّ من معطياتهم أنهم قابلون للإقناع بتجهيزهم للطرح. هؤلاء عملاء خدمة جاهزية الطرح المثاليون.' },
  ];

  const results = await Promise.all(axes.map((a) => huntAxis(a.label, a.category, a.instruction, a.count, dateContext)));
