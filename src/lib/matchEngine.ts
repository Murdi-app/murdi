...
export async function runScopedMatch(args: {
  ...
  try {
    ...
    const prompt = 'أنت محلل تمويل خبير في السوق السعودي والخليجي. ابحث في الويب بحثاً دقيقاً ومهنياً عن المنتجات التمويلية المتاحة حالياً للشركة، وقسّم بحثك إلى ثلاث طبقات بالأولوية:\nالطبقة الأولى (الأهم والأوسع): الجهات السعودية المرخّصة من البنك المركزي السعودي (ساما) — غطِّها بعمق.\nالطبقة الثانية: جهات خليجية قد تموّل شركة سعودية (لها فروع في السعودية أو تموّل عبر الحدود).\nالطبقة الثالثة: جهات تمويل دولية قد تموّل شركات في السعودية أو الأسواق الناشئة.\nالجهات المرجعية لكل طبقة:\n' + LICENSED + '\n' + FOREIGN + '\n\n'
      ...
      + '7ط) ملاءمة القطاع إلزامية: لا ترشّح جهة تموّل قطاعاً لا يعمل فيه العميل — مثل جهة تموّل مشاريع الطاقة لعميل تجزئة. ولا ترشّح مؤسسة تنموية دولية إلا إذا كان لها منتج معلن يقبل طلباً مباشراً من منشأة سعودية بهذا الحجم، لا مجرد تفويض عام.\n'
      + '7ح) يجب أن يستطيع العميل التقديم للجهة مباشرةً. استبعد من يموّل البنوك والمؤسسات المالية لتعيد إقراضه (wholesale أو on-lending) مثل خطوط IFC وFMO وDEG وBII للبنوك، لأن العميل لا يقدّم عليها. وإن أردت ذكرها فاذكر البنك السعودي الموزّع للخط لا الجهة المموّلة له.\n'
      ...
      + '7ب) قاعدة الشمول والتحديث: القوائم أعلاه توضّح شكل الجهات فقط وليست حصراً ولا مرجعاً يُنسخ منه. منتجات التمويل تُطرح وتُوقَف باستمرار، فابحث في الويب عن المطروح اليوم ولا تعتمد على معرفتك المخزّنة. وسّع بحثك لجهات لم تُذكر، ولا تكتفِ بالمشهور؛ فالأقل شهرة قد تكون أعلى احتمالاً للقبول.\n'
  ...
  try {
    ...
    const rows = offers.map((o) => ({
      ...
      fit_score: (() => {
        ...
        const txt2 = (String(o.product || '') + ' ' + String(o.requirements || '') + ' ' + String(o.region || '') + ' ' + String(o.verdict || '')).toLowerCase();
    ...
    for (const r of rows) {
      const k = firmKey(r.provider || '') + '|' + String(r.product || '').toLowerCase().replace(/\s+/g,' ').trim();
