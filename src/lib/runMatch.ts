...
async function searchFundingLayer(layer: 'saudi' | 'gulf' | 'intl', profile: string, licensed: string, targetCount: number): Promise<FundOffer[]> {
  const layerAr = layer === 'saudi' ? 'السعودية (مرخّصة من ساما: بنوك + شركات تمويل + منصات)' : layer === 'gulf' ? 'الخليج (جهات تموّل شركة سعودية عبر فرع في السعودية أو عبر الحدود)' : 'الدولية (بريطانيا + أوروبا + أمريكا + سنغافورة + الصين — تموّل عبر الحدود)';
  ...
  const layerScope = layer === 'saudi'
    ...
    : 'جهات تمويل دولية مقرّها في المملكة المتحدة (بريطانيا) أو دول أوروبا أو الولايات المتحدة الأمريكية أو سنغافورة أو الصين حصراً (بنوك دولية، مؤسسات تمويل تنموية، صناديق دين خاص، منصات تمويل عابرة للحدود، جهات تمويل تجاري ومستخلصات وسلاسل إمداد، مؤسسات تمويل آسيوية تمول الأسواق الناشئة والشرق الأوسط). ممنوع إدراج أي جهة خليجية أو سعودية أو من خارج بريطانيا/أوروبا/أمريكا/سنغافورة/الصين. وقبل الإدراج: تأكد أن الجهة تموّل فعلياً عبر الحدود شركةً سعودية، وطابق منتجها مع نشاط الشركة وحجمها بدقة.';
...
async function searchInvestmentLayer(layer: 'saudi' | 'gulf' | 'intl', profile: string, targetCount: number): Promise<FundOffer[]> {
  ...
  const layerScope = layer === 'saudi'
    ...
    : 'مستثمرون مقرّهم في بريطانيا أو أوروبا أو الولايات المتحدة الأمريكية أو سنغافورة أو الصين حصراً: مستثمرون أفراد (ملائكيون)، صناديق رأس مال جريء (VC)، صناديق نمو، صناديق ملكية خاصة (PE)، محافظ استثمارية، صناديق سيادية ومؤسسية، ومكاتب عائلية. الأولوية القصوى لمن سبق له الاستثمار في السعودية أو الشرق الأوسط ويتطابق قطاعه (اذكر استثماره السابق المشابه في fit)، يليهم من أبدى انفتاحاً على الأسواق الناشئة. سنغافورة بوابة آسيوية نشطة في الشرق الأوسط، والصين عبر صناديقها العالمية المنفتحة على المنطقة. ممنوع إدراج أي مستثمر خليجي أو سعودي هنا.';
  ...
  const prompt = 'أنت باحث استثماري خبير رفيع المستوى يعمل لـ د. عبدالحكيم المرضي. مهمتك: البحث العميق والذكي والواسع في الويب لاكتشاف أكبر عدد من المستثمرين الحقيقيين المناسبين لهذه الشركة ضمن الطبقة الجغرافية المحددة. ابحث براحة وعمق — الكثرة مطلوبة طالما كل مستثمر مناسب فعلاً. لا تحصر نفسك في نوع واحد: ابحث في الصناديق والمكاتب العائلية وصناديق VC وPE والمستثمرين الأفراد (الملائكيين) معاً.\n\n'
    ...
    + '3) مطابقة حجم الجولة (حاسمة إن ذُكرت): إن وُجد "حجم الجولة المطلوبة" في الملف، فطابق المستثمرين الذين يكتب نطاق استثمارهم المعتاد لهذا الحجم تحديداً — لا صندوق ضخم لجولة صغيرة ولا العكس. وراعِ "نوع التمويل المطلوب" (حصة/قرض قابل للتحويل/شريك استراتيجي) في اختيار نوع المستثمر، و"غرض التمويل" في ملاءمة اهتمامه.\n'
    + '4) نشاط حديث: فضل من استثمر خلال آخر 2-3 سنوات.\n'
    + '5) نوع المستثمر الأنسب لوضع الشركة — شخّص أولاً ثم طابق: مستثمر ملائكي فرد أو مسرعة للأفكار والمراحل التأسيسية المبكرة؛ رأس مال جريء (VC) لشركات النمو ذات الإيرادات الصاعدة؛ صناديق النمو والمحافظ الاستثمارية للشركات المستقرة المتوسطة؛ صناديق الملكية الخاصة (PE) للشركات الناضجة الكبيرة؛ المكاتب العائلية والصناديق السيادية/المؤسسية للحصص الاستراتيجية طويلة الأمد. لا تقترح فئة لا تناسب مرحلة الشركة وحجمها فعلياً مهما بدت جذابة.\n\n'
  ...
  if (isDefaulted) { planKind = 'recovery'; }
    ...
    try {
      const invProfile = 'القطاع: ' + (fd?.sector || company?.sector || 'غير محدد')
        ...
        + (fd?.round_size ? ' | حجم الجولة المطلوبة: ' + Number(fd.round_size).toLocaleString() + ' ريال' : '')
        + (fd?.investment_type ? ' | نوع التمويل المطلوب: ' + fd.investment_type : '')
        + (fd?.use_of_funds ? ' | غرض التمويل: ' + fd.use_of_funds : '')
        + (fd?.competitive_edge ? ' | الميزة التنافسية: ' + fd.competitive_edge : '')
