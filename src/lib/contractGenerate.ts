...
export function readClauses(i: ContractInputs, p: ContractPack, scen: ContractScenario[]): ClauseRead[] {
  ...
  // البند الذي يقرّر أي الأبواب تُطرق أصلاً — ولا يُترك سؤالاً معلّقاً
  if (i.assignAllowed === 'no') {
    out.push({
      clause: 'التنازل عن المستحقات',
      yours: 'عقدك يمنعه',
      effect: 'يسقط تسييل المستخلصات وأكثر منصات التمويل معه، لأن قرارها مبنيٌّ على أخذ التنازل. '
        + 'وهذا وحده سبب توقّف أكثر المقاولين — يطرقون هذا الباب ويُردّون ويظنون العيب في ملفهم.',
      ask: 'لا تطلب من المُسنِد تنازلاً. اطلب «توجيه دفع»: أن تُحوَّل مستحقاتك إلى حسابك لدى '
        + 'البنك المموّل. وهو ليس تنازلاً فلا يخالف عقدك، والبنوك تقبله ضبطاً كافياً — '
        + 'والفرق بين الكلمتين هو الفرق بين بابٍ مفتوح وبابٍ مقفل.',
    });
  } else if (i.assignAllowed === 'yes') {
    out.push({
      clause: 'التنازل عن المستحقات',
      yours: 'عقدك يسمح به',
      effect: 'وهذا يفتح لك تسييل المستخلصات ومنصات التمويل المرخّصة — أنسب أداة لحالتك وأسرعها.',
      ask: 'استعمله. وأبلغ الجهة الممولة به من أول رسالة، فهو أقوى ما في ملفك بعد جودة المُسنِد.',
    });
  } else {
    out.push({
      clause: 'التنازل عن المستحقات',
      ...
      effect: 'إن مَنَع العقد التنازل، سقط تمويل المستخلصات كلّه — وهو أنسب أداة لحالتك. '
        + 'ولا يُطرق بابٌ قبل معرفته.',
      ask: 'اقرأ البند اليوم. فإن منعه فالبديل توجيه الدفع لا التنازل، وإن سمح به فهو أقوى ورقة معك.',
    });
  }
...
export function buildContractFile(
  ...
  <p class="mini">وطَرقها بالترتيب مقصود: الجهة الثانية ترى أثر استعلام الأولى. ونحن من يعرض ملفك عليها بتكليفٍ منك ويتابع حتى يصل ردّها — ويبقى التقديم والتعاقد بينك وبينها.</p>`
