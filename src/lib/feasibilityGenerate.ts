...
export async function generateFeasibility(ctx: FeasibilityContext): Promise<{ sections: FeasibilitySections; result: FeasibilityResult; credit: CreditPack; error?: string }> {
  ...
  const pick = (raw: string, need = 'executiveSummary'): Record<string, unknown> | null => {
    ...
    const keys = ['executiveSummary', 'marketStudy', 'competition', 'swot', 'operationalPlan', 'legalCompliance', 'economicImpact', 'technicalStudy', 'facilitySizing', 'securityStructure', 'assumptionsNote', 'risks', 'conclusion', 'funderQA', 'facts'];
  ...
  for (const k of ['technicalStudy', 'operationalPlan', 'legalCompliance', 'economicImpact', 'facilitySizing', 'securityStructure', 'assumptionsNote', 'risks', 'conclusion', 'funderQA']) if (b && b[k]) bOnly[k] = b[k];
...
function empty(): FeasibilitySections {
  return { executiveSummary: '', marketStudy: '', competition: '', swot: '', operationalPlan: '', legalCompliance: '', economicImpact: '', technicalStudy: '', facilitySizing: '', securityStructure: '', assumptionsNote: '', risks: '', conclusion: '', funderQA: '', sources: [] };
}
