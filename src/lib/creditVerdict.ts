...
export function verdictOf(cap: Capacity | null, comp: string[], blockers: Blocker[], readyDoors: number): Verdict {
  ...
  // كان هذا الشرط يقرأ `blockers.length === 0` — أي أنّ نقصاً مسجّلاً عند
  // جهةٍ بعيدة يُسقط حكم من عنده أبوابٌ نظيفة اليوم. وذلك خطأ في القراءة:
  // العميل لا يطرق الجهات كلها، بل يطرق أوّلها. فمن عنده أبوابٌ بلا نواقص
  // وقدرةٌ تغطّي مبلغه والتزامٌ نظامي مكتمل فهو ممول الآن، وبقية النواقص
  // تُذكر في موضعها: توسيعُ خيارات لا شرطُ دخول. والحكم بـ«بشرط» على مثل
  // هذا يبخس ملفَه أمامه، ويُسقط الوثيقة إن طرق باباً فمُوِّل من أوّل جولة.
  if (readyDoors > 0 && (cap.ratio === null || cap.ratio >= 1)) return {
    kind: 'now',
    headline: 'ملفك قابل للتمويل الآن',
    because: 'قدرتك تغطي المبلغ، والتزامك النظامي مكتمل، و' + readyDoors
      + ' من جهاتك لم تُسجَّل عليك عندها أي نواقص — فهذه مفتوحة لك اليوم.'
      + (blockers.length > 0
        ? ' وبقية جهاتك تطلب ' + blockers.length + (blockers.length === 1 ? ' نقصاً' : ' نواقص')
          + ' مذكورة في قائمة الإصلاح، وإصلاحها يوسّع خياراتك ولا يفتح أوّلها.'
        : '')
      + ' وما ينقصك ليس تأهيلاً بل ملفاً مكتوباً وطَرقاً بالترتيب الصحيح.',
  };
...
export function buildCreditVerdict(
  ...
  // ثلاثة بالبطاقات، ثم اثنتا عشرة بالجدول، والبقية تُعدّ ولا تُسمّى.
  //
  // وسبب الحدّ تجاريّ لا تجميليّ: صفٌّ فيه تسعون جهةً «مؤهَّلة» يقرؤه صاحبه
  // «تسعون باباً مفتوحاً لي — فلمَ أشتري الدراسة؟»، وهو في الحقيقة ترتيبُ
  // محرّكٍ لم يُدرَس صفٌّ منه بيد. فتُعطى الوثيقة خمس عشرة جهةً مرتَّبةً
  // بالاسم — وهي أكثر ممّا يطرقه أحدٌ في جولة — ويُقال للباقي عددُه وأين
  // يُدرَس. والعدد يبقى ظاهراً فلا يُخفى عنه شيء.
  const REST_SHOWN = 12;
  const restAll = ranked.slice(3);
  const rest = restAll.slice(0, REST_SHOWN);
  const restHidden = restAll.length - rest.length;
  ...
  ${rest.length ? `<h2>٤ · بقية جهاتك المؤهَّلة</h2>
    <p class="mini">${restAll.length === rest.length
        ? rest.length + ' جهة أخرى انطبقت شروطها على ملفك. هذه أسماؤها وحدودها — وترتيبها بعد الثلاثة الأولى.'
        : 'انطبقت شروط ' + restAll.length + ' جهة أخرى على ملفك، وهذه أقواها ' + rest.length
          + ' مرتّبةً بعد الثلاثة الأولى. والباقي — ' + restHidden + ' جهة — تُدرَس كل واحدة منها على حدة في الدراسة الاقتصادية والائتمانية الكاملة: حدودها وشروطها وباب الدخول إليها. '
          + 'ولم تُسرد هنا لأن سردها اسماً بلا دراسةٍ يُغري بطَرقها كلها، وطَرقها كلها يُفسد ملفك — فالجهة التالية ترى أثر استعلام سابقتها.'
      }</p>
    <table>
  ...
  <div class="stop">
    هذا حكمٌ يُعرّفك بموقفك وأبوابك. ولا يشمل: بناء ملفك التمويلي بالعربية والإنجليزية · عرضه على الجهات بتكليف خطّي منك · متابعة الردود · شرح الشروط والتفاوض عليها.
...
@media print{body{background:#fff;padding:0}.doc{border:none;border-radius:0;padding:0}.door{break-inside:avoid}
  h2,table,.door,.verdict,.key,.stop{break-inside:avoid}}
`;
