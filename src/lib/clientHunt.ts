...
async function huntCallListAxis(dateContext: string, dedupContext: string): Promise<CLead[]> {
  const prompt = 'أنت باحث ميداني خبير تجهّز قائمة اتصال يومية لفريق مبيعات نسائي يعمل لمنصة مُرضي (murdi.sa) — منصة سعودية لقياس ورفع جاهزية الشركات للحصول على رأس المال، تتبع حلول المرضي للاستشارات المالية.\n\n'
    ...
    + '2) الجوال المحمول شرط قبول نهائي: لا تُدرج أي منشأة إلا برقم جوال سعودي محمول منشور (يبدأ 05 أو 9665) من موقعها أو حسابها الموثق أو دليل أعمال موثوق. الهواتف الثابتة والأرقام الموحدة مرفوضة. لا تخترع رقماً أبداً — الاختراع خيانة للمهمة وللموظفة التي ستتصل.\n'
...
export async function runClientHunt(): Promise<{ total: number }> {
  ...
  const axes = [
    ...
    { sectors: 'اللوجستيات والنقل والتخزين، السياحة والضيافة والفعاليات، الزراعة والثروة الحيوانية، ورش السيارات ومراكز الخدمة', count: 35 },
  ];
